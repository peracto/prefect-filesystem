...
https://github.com/organizations/peracto/repositories/new

Upon creation, push the repository to GitHub:
```bash
git remote add origin https://github.com/peracto/prefect-filesystem.git
git branch -M main
git push -u origin main
```

It's recommended to setup some protection rules for main at:
https://github.com/peracto/prefect-filesystem/settings/branches
...
```markdown
::: prefect_filesystem.{module_name}
```
...
Upon publishing, a `docs` branch is automatically created. To hook this up to GitHub Pages, simply head over to https://github.com/peracto/prefect-filesystem/settings/pages, select `docs` under the dropdown menu, keep the default `/root` folder, `Save`, and upon refresh, you should see a prompt stating "Your site is published at https://peracto.github.io/prefect-filesystem". Don't forget to add this link to the repo's "About" section, under "Website" so users can access the docs easily.
